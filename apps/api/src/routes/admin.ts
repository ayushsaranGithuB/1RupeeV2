...
// Campaign Supporters
admin.get('/campaigns/:campaignId/supporters', async (c) => {
    try {
        ...
        return c.json(successResponse(supporters));
    } catch (error: any) {
        console.error('Error fetching campaign supporters:', error.message);
        return c.json(errorResponse('INTERNAL_ERROR', 'Failed to fetch supporters'), 500);
    }
});
